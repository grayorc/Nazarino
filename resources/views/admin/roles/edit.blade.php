                ...
                <form class="form-horizontal" action="{{ route('admin.roles.update', $role->id) }}" method="POST">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="name" class="form-control" id="name" placeholder="نام نقش را وارد کنید" value="{{ old('name', $role->name) }}">
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="display_name" class="form-control" id="display_name" placeholder="نام نمایشی نقش را وارد کنید" value="{{ old('display_name', $role->display_name) }}">
                            @error('display_name')
                                <div class="text-danger mt-2">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <textarea name="description" class="form-control" id="description" placeholder="توضیحات نقش را وارد کنید">{{ old('description', $role->description) }}</textarea>
                            @error('description')
                                <div class="text-danger mt-2">{{ $message }}</div>
                            @enderror
                        </div>
                                ...
                                @foreach($permissions as $permission)
                                    <div class="col-md-3">
                                        <div class="custom-control custom-checkbox">
                                            <input type="checkbox" class="custom-control-input" name="permissions[]" value="{{ $permission->id }}" id="permission_{{ $permission->id }}" {{ in_array($permission->id, $role->permissions->pluck('id')->toArray()) ? 'checked' : '' }}>
                    ...
                    <div class="card-footer">
                        <button type="submit" class="btn btn-info">بروزرسانی نقش</button>
