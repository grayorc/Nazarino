<x-min-layout>
    <div class="flex relative min-h-screen bg-gray-100 dark:bg-PrimaryBlack">
        ...
        <section class="container px-4 mx-auto flex-grow-1 pb-32">
            <form action="{{ route('elections.update', $election) }}" method="PUT" class="flex flex-col mt-6 bg-SecondaryBlack rounded-lg p-5" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="flex flex-row">
                    <div class="">
                        ...
                        <input type="text" name="title" value="{{ $election->title }}" placeholder="عنوان نظرسنجی را بنویسید" class="block  mt-2 w-full placeholder-gray-400/70 dark:placeholder-gray-500 rounded-lg border border-gray-200 bg-white px-5 py-2.5 text-gray-700 focus:border-blue-400 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-40 dark:border-gray-600 dark:bg-PrimaryBlack dark:text-gray-300 dark:focus:border-blue-300" />
                    </div>
                </div>
                <div class="flex flex-row">
                </div>
                ...
                <div class="mt-5">
                    ...
                    <label
                        style="background-image: url('{{ Storage::url($election->image?->path) }}');"
                        for="dropzone-file"
                        class="relative bg-cover bg-center bg-no-repeat image flex flex-col items-center w-full max-w-lg p-5 mx-auto mt-2 text-center bg-white border-2 border-gray-300 border-dashed cursor-pointer dark:bg-PrimaryBlack dark:border-gray-700 rounded-xl">
      ...
      initCalendar() {
        ...
        this.datePickerYear = {{ verta($election->end_date)->format('Y') }};
        this.datePickerMonth = {{ verta($election->end_date)->format('m') }};
        this.datePickerDay = {{ verta($election->end_date)->format('d') }};
        ...
        this.datePickerCalculateDays();
                ...
                <div class="mt-5">
                    <div class="flex ml-auto">
                        <div dir="ltr" x-data="{ switchOn: {{ $election->has_comment }} }" class="flex items-center justify-center space-x-2">
                ...
                <div class="mt-5">
                    <div class="flex ml-auto">
                        <div dir="ltr" x-data="{ switchOn: {{ $election->is_public }} }" class="flex items-center justify-center space-x-2">
