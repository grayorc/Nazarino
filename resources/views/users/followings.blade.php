        ...
        <div class="flex flex-col w-11/12 lg:w-10/12 mx-auto">
            ...
            <div class="mb-8">
                <div class="flex items-center gap-4">
                    ...
                    <h1 class="text-2xl md:text-3xl font-bold text-PrimaryBlack dark:text-primaryWhite">دنبال‌شده‌های {{ $user->username }}</h1>
                </div>
                <p class="mt-2 text-SecondaryBlack/90 dark:text-SecondaryWhite/90">افرادی که {{ $user->username }} آنها را دنبال می‌کند</p>
            </div>
            ...
            @if($followings->isEmpty())
                <div class="flex flex-col items-center justify-center p-8 bg-white/50 dark:bg-zinc-800/50 rounded-2xl border border-gray-200 dark:border-gray-700">
                    ...
                    <h3 class="text-xl font-medium text-PrimaryBlack dark:text-primaryWhite">هیچ دنبال‌شده‌ای یافت نشد</h3>
            ...
            @else
                        ...
                        <div class="bg-gradient-to-br from-white to-primaryWhite/90 dark:from-Sidebar_background dark:to-Chart_background shadow-md rounded-xl border border-gray-200/50 dark:border-Sidebar_background_hover/30 p-4">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="w-12 h-12 rounded-full overflow-hidden border-2 border-white dark:border-gray-700">
                                    @if($following->image)
                                        <img src="{{ asset('storage/' . $following->image->path) }}" alt="{{ $following->username }}" class="w-full h-full object-cover">
                                    @else
                                ...
                                <!-- User Info -->
                                <div class="flex-1">
                                    <a href="{{ route('users.profile', $following->username) }}" class="font-bold text-PrimaryBlack dark:text-primaryWhite hover:text-primaryColor dark:hover:text-primaryColor/90">
                                        {{ $following->getDisplayNameAttribute() }}
                                    </a>
                                    <p class="text-sm text-gray-600 dark:text-gray-400">{{ $following->username.'@' }}</p>
                                </div>

                                <!-- Follow Button -->
                                <div id="follow-button-container-{{ $following->id }}">
                                    @php
                                        $isFollowing = Auth::check() ?
                                            \App\Models\Follower::where('user_id', $following->id)
                                                ->where('follower_id', Auth::id())
                                                ->exists() : false;
                                        $followersCount = $following->countFollowers();
                                    @endphp
                                    ...
                                    @auth
                                        @if(Auth::id() !== $following->id)
                                            <button
                                                ...
                                                hx-post="{{ route('users.follow', $following->username) }}"
                                                hx-headers='{"x-csrf-token": "{{ csrf_token() }}"}'
                                                hx-trigger="click"
                                                hx-target="#follow-button-{{ $following->id }}"
                                                hx-swap="none"
                                            ...
                                            <script>
                                                document.body.addEventListener('htmx:afterRequest', function(event) {
                                                    if (event.detail.elt.id === 'follow-button-{{ $following->id }}') {
                                                        const response = JSON.parse(event.detail.xhr.responseText);
                                                        const button = document.getElementById('follow-button-{{ $following->id }}');
                ...
                <div class="mt-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div class="text-sm text-gray-500 dark:text-gray-400">
                        صفحه <span class="font-medium text-gray-700 dark:text-gray-100">{{ $followings->currentPage() }} از {{ $followings->lastPage() }}</span>
                    </div>

                    <div class="flex items-center gap-x-4">
                        <a href="{{ $followings->previousPageUrl() }}" class="flex items-center justify-center w-1/2 px-5 py-2 text-sm text-gray-700 capitalize transition-colors duration-200 bg-white border rounded-md sm:w-auto gap-x-2 hover:bg-gray-100 dark:bg-PrimaryBlack dark:text-gray-200 dark:border-gray-700 dark:hover:bg-SecondaryBlack {{ $followings->onFirstPage()?"pointer-events-none opacity-40":""}}">
                        ...
                        <a href="{{ $followings->nextPageUrl() }}" class="flex items-center justify-center w-1/2 px-5 py-2 text-sm text-gray-700 capitalize transition-colors duration-200 bg-white border rounded-md sm:w-auto gap-x-2 hover:bg-gray-100 dark:bg-PrimaryBlack dark:text-gray-200 dark:border-gray-700 dark:hover:bg-SecondaryBlack {{ $followings->onLastPage()?"pointer-events-none opacity-40":""}}">
                        <span>
                            صفحه بعد
                        </span>
